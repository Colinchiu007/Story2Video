...
serve(async (req) => {
  ...
  const model = (body.model as string) ?? "viduq3-pro";
  ...
  const viduBody: Record<string, unknown> = {
    ...
    duration: Math.min(seconds, 16),
  };

  if (inputReferenceUrl && mode === "image-to-video") {
    viduBody.images = [inputReferenceUrl];
  }

  try {
    const upstream = await fetch("https://api.vidu.cn/ent/v2/vidu2video", {
      method: "POST",
      headers: {
    ...
    if (!upstream.ok) {
      ...
      return new Response(
        JSON.stringify({ error: `Vidu 视频生成失败: ${msg}` }),
        { status: 200, headers: { "Content-Type": "application/json", ...corsHeaders() } },
      );
    }
    ...
    if (taskId) {
      return new Response(
        JSON.stringify({ videoId: String(taskId), status: result.state ?? "created" }),
        { status: 200, headers: { "Content-Type": "application/json", ...corsHeaders() } },
      );
    }
