...
serve(async (req) => {
  ...
  try {
    const upstream = await fetch(`https://api.vidu.cn/ent/v2/tasks/${videoId}/creations`, {
      method: "GET",
      headers: {
    ...
    if (state === "success") {
      ...
      if (first?.url) {
        publicUrl = await streamVideoToStorage(String(first.url));
      }
      return new Response(
        JSON.stringify({
          status: "completed",
          progress: 100,
          video_url: publicUrl ?? first?.url ?? null,
