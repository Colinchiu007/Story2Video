...
| 2 | 42 ??????????30 ? RPM ?? | 85% ??????????? | **?????** |
| 3 | ????????????????? | ?????????????? | **????** |
| 4 | splitTextToScenesSmart ????????????? | 3 ??????? | **??????** |
| 5 | IMAGE_DELAY_MS const ???? | ??????????? | **??????** |
| 6 | ????????? API ???? | 5 ?????? | **????**??????? |

### ????

#### 1. ??? ? MiniMax ??? isImageGenerationAvailable()

**??**?`isImageGenerationAvailable()` ???? `kling/jimeng/vidu/sensenova`?? IMAGE_OPTIONS ?? 6 ?????`minimax` ????

**??**?????????? UI ????????????image-generation.ts???????????????????????????UI ?? ? ????? ? ?????

**??**????????????????????????????????????????????? if-else?

#### 2. ????? ? Promise.all ????????

**??**?`Promise.all(imagePromises)` ??????? 42 ????????? 7 ??????? RPM ???

**??**???????????? `.map` + `Promise.all`??????? API ??????

**??**????? API ??????????????????????????????? `batchParallel` + ?????????? `Promise.all`?

#### 3. ???? ? ????????

**??**?`.env` ????? `VITE_SENTENCE_SPLITTER_URL` ? `VITE_PROMPT_ENGINE_URL`?`isAvailable()` ???? false?smart ??????????

**??**?`.env.example` ?????"?????????? TS ??"?????????????? sentence-splitter ??????8000?? example ??????8014?????
...
**??**?`splitTextToScenes` ? `splitTextToScenesSmart` ?? `if (!trimmed) return [];`????????????

**??**??????????????????????

**??**??? AST ????????? `@babel/codemod` ? IDE ?????????????????????????

#### 5. ?????? ? const ??? if ??????

**??**???????? gallery ? batch ?? if ?????? `const IMAGE_DELAY_MS = 1500;`??? const ????????
...
- ???? API ???????? batchParallel?? concurrency + delayMs?
- ??? Promise.all ???? API
- ????????????????????????
```
...
- ?? `ts-prune` ??????????? `isAvailable()` ??????????
- `batchParallel` ???????? Promise.all ? lint ??
